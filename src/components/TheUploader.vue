...
export default class TheUploader extends Vue {
  // * === === === === === === === === === ===
  // * Vuex 💧 💧 💧 💧 💧 💧 💧 💧 💧 💧 💧
  ...
  get dropzoneOptions() {
    ...
    return {
      ...
      parallelUploads: 1,
      success: (f: object, response: Docs.Record) =>
  ...
  refreshCta() {
    const uploaderCta = this.$text.UploaderCta
    this.dropzone.$el.innerHTML = `
      <div class="dropzone__bg">&nbsp;</div>
      <div class="dropzone__cta"><i aria-hidden="true"
      class="v-icon notranslate drawer-icon material-icons theme--light grey--text text--darken"
      style="font-size:150px; opacity:0.2;pointer-events: none">cloud_upload</i>
      <span class="cta-text">${uploaderCta}</span></div>
    `
  }
      ...
      <div class="files-container">
        <div class="file" v-for="file in files" :key="file.upload.uuid">
          ...
          <div class="file__text">
            ...
            <!-- <span v-text="file.upload.filename" />
            <span v-text="getFileProgress(file)" />-->
          </div>
          <v-btn
            icon
            class="file__action"
...
#dropzone {
  ...
  .dropzone__bg {
    ...
    background-size: 174px 100.46px;
